...
public class TurtleSoup {
    ...
    public static double calculateHeadingToPoint(double currentHeading, int currentX, int currentY,
        ...
        double res = 90 - currentHeading - Math.atan2(targetY - currentY, targetX - currentX) / Math.PI * 180;
        if (res < 0) {
            res += 360;
        }
        return res;
    ...
    public static List<Double> calculateHeadings(List<Integer> xCoords, List<Integer> yCoords) {
        ...
        for (int i = 1; i < xCoords.size(); i++) {
            double degree = calculateHeadingToPoint(heading, xCoords.get(i-1), yCoords.get(i-1), xCoords.get(i), yCoords.get(i));
            res.add(degree);
            heading += degree;
        }
        return res;
